...
try:
    from pyrrm.models.numba_kernels import NUMBA_AVAILABLE, _gr6j_core_numba
except ImportError:
...
class GR6J(BaseRainfallRunoffModel):
    ...
    def run(self, inputs: pd.DataFrame) -> pd.DataFrame:
        ...
        core_fn = _gr6j_core_numba if NUMBA_AVAILABLE else _gr6j_core
        flow, prod_final, rout_final, exp_final, uh1_final, uh2_final = core_fn(
            self._params['X1'],
            self._params['X2'],
            self._params['X3'],
            self._params['X4'],
