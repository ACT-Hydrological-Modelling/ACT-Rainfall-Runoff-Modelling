...
# ## Purpose
#
# This notebook runs systematic batch calibration experiments for headwater
# gauges in the Lower Burley Griffin (LBG) catchment network. It loads
# **SILO area-weighted daily climate data** (precipitation and Morton wet
# areal PET), configures experiments, and executes them using pyrrm's
# `ExperimentList` and `BatchExperimentRunner`.
#
# ## What You'll Learn
#
# - How to load SILO pre-aggregated climate timeseries for each subcatchment
# - How to read catchment areas from the LBG subcatchments shapefile
# - How to build per-gauge `ExperimentList` configurations
...
# - SILO timeseries CSVs at `pywr-ACT/data/SILO/timeseries/`
# - Observed flow CSVs under `data/{gauge_id}/`
#
# ## Estimated Time
#
# - Data prep: ~1 minute
...
# | 2 | Data discovery | Map gauges to SILO CSVs and observed flow files |
# | 3 | Data cleaning and preparation | Load SILO climate, read areas from shapefile, clean flow, align |
...
#
# Climate data comes from **SILO area-weighted daily timeseries** (one CSV per
# subcatchment). Observed flow files remain in the existing `data/{gauge_id}/`
# folders.
#
# | Source | Pattern | Example |
# |--------|---------|---------|
# | SILO climate | `subcatch_{gauge_id}.0.csv` | `subcatch_410705.0.csv` |
# | Observed flow | `*_observed_flow.csv`, `*_output_SDmodel.csv`, `*_recorded_Flow.csv` | `410705_observed_flow.csv` |
...
def discover_gauge_files(
    ...
    Returns a list of dicts with keys: gauge_id, silo_file, flow_file.
    """
    gauges = []
...
# For each gauge we:
#
# 1. Read the SILO area-weighted daily CSV (`precip`, `pet_mwet`).
# 2. Read catchment areas from the LBG subcatchments shapefile DBF.
# 3. Read the observed flow CSV, auto-detect the value column, replace sentinels
#    (`-9999`) with NaN, drop negative flows.
# 4. Inner-join climate and flow on the date index so all series are aligned.
# 5. Return `(inputs_df, observed_array, metadata_dict)`.
...
def load_catchment_areas_from_dbf(dbf_path: Path) -> Dict[str, float]:
    ...
    The DBF has two fields: ``fid`` (gauge ID as string) and ``area_km2``.
...
def load_silo_climate(filepath: Path) -> pd.DataFrame:
    ...
    Reads the ``precip`` and ``pet_mwet`` (Morton wet areal PET) columns,
    renames them to ``precipitation`` and ``pet``, and sets a DatetimeIndex.
    """
...
def prepare_gauge_data(
    ...
    Loads SILO climate data and observed flow, inner-joins, and returns
    ``(inputs_df, observed_array, metadata)``.
    """
...
for g in gauge_inventory:
    gauge_id = g['gauge_id']
    try:
        inputs_df, observed, meta = prepare_gauge_data(g, CATCHMENT_AREAS_ALL)
        ...
        print(f"  {gauge_id}: {meta['n_days']} days  "
              ...
              f"Area={area_str}  [SILO: {meta['silo_file']}]")
...
print("Catchment areas (from LBG_subcatchments shapefile):")
...
# |---------|-----------|-------------|
# | **SILO climate data** | `load_silo_climate()` | Load pre-aggregated area-weighted daily P and PET |
# | **Shapefile areas** | `load_catchment_areas_from_dbf()` | Read catchment areas from LBG subcatchments shapefile |
# | **Data discovery** | `discover_gauge_files()` | Map gauge IDs to SILO CSVs and observed flow files |
...
# ### Key Takeaways
#
# 1. Climate inputs use SILO area-weighted daily timeseries (precipitation and
#    Morton wet areal PET) -- one pre-aggregated CSV per subcatchment.
# 2. Catchment areas are read directly from the LBG subcatchments shapefile DBF.
